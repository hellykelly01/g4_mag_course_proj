...
G4RotationMatrix* DetectorConstructionBase::CalcRotation(const G4ThreeVector& targetPosition){
...
G4VPhysicalVolume* DetectorConstructionBase::Construct(){
  ...
  G4Sphere* solidPM = new G4Sphere("PhotoMultiplier", 0.0, sphereRadius, sliceStartPhi, sliceEndPhi, sliceStartAngle, sliceEndAngle);
  G4LogicalVolume* logicPM = new G4LogicalVolume(solidPM, fLAB, "PhotoMultiplier");
  logicPM->SetVisAttributes(new G4VisAttributes(G4Colour(1.0, 0.0, 1.0)));
  for(G4ThreeVector& vec : vertices){
    vec.setMag(1.0);
    new G4PVPlacement(CalcRotation(vec), vec * 0.866 * m, logicPM, "PhotoMultiplier", logicLAB, false, 0); 
  }
